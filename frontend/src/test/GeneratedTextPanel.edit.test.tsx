...
describe('GeneratedTextPanel - 編集機能', () => {
  ...
  it('生成テキストが表示される', () => {
    ...
    expect(
      screen.getByDisplayValue('AI生成されたテキスト')
    ).toBeInTheDocument();
  });
  ...
  it('保存エラー時にエラートーストが表示される', async () => {
    ...
    (updateWorkOrderEditedText as any).mockRejectedValue(
  ...
  it('編集済みテキストがある場合、編集済み表示が出る', () => {
    render(
      <GeneratedTextPanel {...defaultProps} editedText="編集済みテキスト" />
    );

    // 編集済み表示の確認
    expect(
      screen.getByText('✓ 編集済み - この内容は編集されています')
    ).toBeInTheDocument();
  ...
  it('編集済みテキストがある場合、それが生成テキストより優先される', () => {
    ...
    expect(screen.getByDisplayValue('編集済みテキスト')).toBeInTheDocument();
    expect(
      screen.queryByDisplayValue('AI生成テキスト')
    ).not.toBeInTheDocument();
  });
