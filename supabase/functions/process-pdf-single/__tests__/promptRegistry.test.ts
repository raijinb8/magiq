...
describe('promptRegistry', () => {
  ...
  describe('プロンプトレジストリの整合性', () => {
    it('すべてのエントリが一意のバージョンを持つ', () => {
      ...
      assertEquals(duplicates.length, 0, `重複するバージョン: ${duplicates.join(', ')}`)
    })
    ...
    it('すべてのプロンプト関数が有効な文字列を返す', () => {
      Object.entries(PROMPT_REGISTRY).forEach(([key, entry]) => {
        if (entry) {
          const result = entry.promptFunction('test.pdf')
          assertNotEquals(result, '', `${key} のプロンプト関数が空文字列を返しました`)
          assertNotEquals(result, null, `${key} のプロンプト関数がnullを返しました`)
          assertNotEquals(result, undefined, `${key} のプロンプト関数がundefinedを返しました`)
        }
      })
    })
  })
