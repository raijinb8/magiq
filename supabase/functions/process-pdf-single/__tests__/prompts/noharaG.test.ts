// noharaG/misawa プロンプトのテスト
import { assertEquals, assertStringIncludes } from "@std/testing/asserts";
import { describe, it } from "@std/testing/bdd";
import { NOHARA_G_MISAWA_PROMPT } from "../../prompts/noharaG/misawa.ts";

describe("NOHARA_G_MISAWA_PROMPT", () => {
  it("ファイル名を含むプロンプトを生成する", () => {
    const fileName = "test_work_order.pdf";
    const prompt = NOHARA_G_MISAWA_PROMPT(fileName);
  ...
  it("必要な指示が含まれている", () => {
    const prompt = NOHARA_G_MISAWA_PROMPT("test.pdf");
  ...
  it("異なるファイル名で異なるプロンプトを生成する", () => {
    const prompt1 = NOHARA_G_MISAWA_PROMPT("file1.pdf");
    const prompt2 = NOHARA_G_MISAWA_PROMPT("file2.pdf");
  ...
  it("空のファイル名でもエラーにならない", () => {
    const prompt = NOHARA_G_MISAWA_PROMPT("");
    assertEquals(typeof prompt, "string");
    assertStringIncludes(prompt, "野原Ｇ住環境");
  });
  ...
  it("特殊文字を含むファイル名を正しく処理する", () => {
    ...
    const prompt = NOHARA_G_MISAWA_PROMPT(specialFileName);
