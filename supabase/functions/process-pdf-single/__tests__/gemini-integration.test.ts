...
describe('Gemini API モック統合デモ', () => {
  ...
  describe('会社別レスポンスのモック', () => {
    it('NOHARA_G_MISAWA用のレスポンス', async () => {
      mockGeminiAI = createMockGeminiAI()

      // 条件付きレスポンスを設定
      mockGeminiAI.setConditionalResponse(request => {
        const requestStr = JSON.stringify(request)
        return requestStr.includes('NOHARA_G_MISAWA') || requestStr.includes('野原')
      }, GEMINI_MOCK_RESPONSES.NOHARA_G_MISAWA)

      // 野原G住環境_ミサワホーム用のリクエスト
      const response = await mockGeminiAI.models.generateContent({
        model: 'gemini-2.5-flash-preview-04-17',
        contents: [
          {
  ...
  describe('API呼び出しのトラッキング', () => {
    it('API呼び出し回数とリクエスト内容の追跡', async () => {
      ...
      // 複数回のAPI呼び出し
      const requests = [{ text: '1回目のリクエスト' }, { text: '2回目のリクエスト' }, { text: '3回目のリクエスト' }]
...
【備考・注意事項】モックテスト用のレスポンス`,
      ...
      const dbRecord = {
        ...
        prompt_identifier: 'NOHARA_G_MISAWA_V20250526',
