// バージョン履歴:
// V20250519: 初版作成
// V20250526: 日付表示、不要キャッシュ使用問題、（PFX・担当者・得意先）読み取りエラーに対応
// V20250610: NOHARA_GからNOHARA_G_MISAWAに移行
...
* **担当会社・担当者:** (「野原グループ株式会社 住環境カンパニー」を「野原Ｇ住環境」に置換。担当者の苗字抽出。両者を**全角スペース**で連結。) 例: 「野原Ｇ住環境　牧」「野原Ｇ住環境　黒田」
...
* **資材明細:** (プレフィックス決定[バリア強/バリア/強(全角)]。「FVTB」はプレフィックスなし、「VTBZ」は「強」（全角）、「バリア石膏ボード」は「バリア」（全角）、「バリア付強化石膏ボード」は「バリア強」（全角）に変換します。プレフィックスがない場合は何もつけません。
...
export const NOHARA_G_MISAWA_COMPANY_INFO = {
  ...
  notes: ['野原G住環境から移行（旧NOHARA_G_MISAWA）']
} as const;